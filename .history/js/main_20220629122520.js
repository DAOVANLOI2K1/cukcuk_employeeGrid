...
for (let i = 1; i < document.querySelectorAll('table tr').length; i++) {
    if(document.getElementsByClassName('.checkbox')[i-1].ischecked) {
        document.querySelectorAll('table tr')[i].classList.add('active');
    }
    else {
        document.querySelectorAll('table tr')[i].classList.remove('active');
    }
}