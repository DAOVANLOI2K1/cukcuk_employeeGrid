class EmployeePage {
    ...
    getValueRow() {
        ...
        Object.keys(data).filter(function(keyName) {
            data[keyName] = me.grid.find("tr.active td")[i].innerText;
            if(keyName === "Gender" || keyName === "WorkStatus") {
                switch(keyName) {
                    case resources.Enum.Gender:
                        data[keyName] = me.convertEnumGender(data[keyName]);
                        break;
                    case resources.Enum.WorkStatus:
                        data[keyName] = me.convertEnumWorkStatus(data[keyName]);
                        break;
                }
            }
            i++;
    ...
    // Hàm convert data enum giới tính
    convertEnumGender(data) {
        let me = this;

        if(data) {
            switch(data) {
                case resources.gender[0]:
                    data = 0;
                    break;
                case resources.gender[1]:
                    data = 1;
                    break;
                case resources.gender[2]:
                    data = 2;
                    break;
                case resources.gender[3]:
                    data = 3;
                    break;
            }
        }
    ...
    // Hàm convert data enum tình trạng công việc
    convertEnumWorkStatus(data) {
        let me = this;

        if(data) {
            switch(data) {
                case resources.workStatus[0]:
                    data = 0;
                    break;
                case resources.workStatus[1]:
                    data = 1;
                    break;
                case resources.workStatus[2]:
                    data = 2;
                    break;
            }
        }
