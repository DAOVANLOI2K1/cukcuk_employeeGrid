class EmployeePage {
    ...
    getData() {
        ...
        let me = this,
            ...
            console.log(me.count_show.find(".pagging_number.active").innerText);

        let data = CommonFn.Ajax(me.Url + "?pageSize = " + pageSize + "&pageNumber = " + pageNumber, resources.Method.Get, {}, function(response) {
            if(response) {
                me.loadData(response, pageSize);
            }
            else {
                console.error("Có lỗi trong quá trình lấy dữ liệu!");
            }
    ...
    renderData(data, pageSize) {
        ...
        me.grid.html(table);
        me.showCountEmployee(data, pageSize);
    }
    ...
    showCountEmployee(data, pageSize) {
        let me = this,
            sl = data.totalCount;

        me.count_show.find(".count_datatable")[0].innerText = `01-${pageSize}/${sl}`;
        me.count_show.find(".count_datatable")[1].innerText = `${pageSize} lao động/trang`;
    }
}
