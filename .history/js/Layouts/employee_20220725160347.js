class EmployeePage {
    ...
    delete() {
        let me = this,
            ...
            activeCode = $(activeRow.find("td")[1]).text();

        // Xử lý khi khách hàng thao tác xóa nhân viên
        if(activeRow.length != 0) {
            if(confirm(`Bạn có chắc muốn xóa nhân viên có mã la ${activeCode} này?`)) {
                // Thực thi xóa nhân viên
                me.deleteEmployee(activeRow, activeID);
            } else {
                alert("Đã hủy xóa!");
            }
        } else {
            alert("Không thể thực hiện xóa vì bạn chưa chọn nhân viên nào!");
        }
    }

    // Hàm gọi Api xóa nhân viên
    deleteEmployee(activeRow, activeID) {
        let me = this;
        ...
        CommonFn.Ajax(me.Url + '/' + activeID, resources.Method.Delete, {}, function(response) {
            if(response) {
                ...
                alert("Có lỗi trong việc xóa nhân viên này!");
            }
        });
        // Xóa record được chọn
        activeRow.remove();
}
    ...
    initEventTable() {
        ...
        // Sự kiện double_click table
        me.grid.off("dblclick", "tr");
        me.grid.on("dblclick", "tr", function() {
            if(me["update"] && typeof me["update"] === "function") {
    ...
    initEventArrowPaging() {
        ...
        me.count_show.find(".navigate_icon").on("click", function() { 
            ...
            // Kiểm tra điều kiện value   
            if(value < pageNumberCount - 1) {
                // Gọi hàm chứa sự kiện next grid
                me.nextGrid(value, pageNumberCount, this);
            }
            if(value > 0) {
                // Gọi hàm chứa sự kiện prev grid
                me.prevGrid(value, this);
            }             
            // Lấy dữ liệu phân trang và lọc
            me.getData(show_option);
        });
    ...
    nextGrid(value, pageNumberCount, control) {
        let me = this;

        // Sự kiện click arrow next
        if($(control).hasClass("next")) {
            value++;
            me.count_show.find(".pagging_number").removeClass("active");
            me.count_show.find(`.pagging_number[value = "${value}"]`).addClass("active");
        }
        // Sự kiện click arrow next_all
        else if($(control).hasClass("next_all")) {
    ...
    // Hàm chứa sự kiện click arrow prev and prev_all
    prevGrid(value, control) {
        let me = this;

        // Sự kiện click arrow prev
        if($(control).hasClass("prev")) {
            value--;
            me.count_show.find(".pagging_number").removeClass("active");
            me.count_show.find(`.pagging_number[value = "${value}"]`).addClass("active");
        }
        // Sự kiện click arrow prev_all
        else if($(control).hasClass("prev_all")) {
            value = 0;
            me.count_show.find(".pagging_number").removeClass("active");
            me.count_show.find(`.pagging_number[value = "${value}"]`).addClass("active");
        }
