class FormNewEmployee {
        ...
        if(data && param.formMode === "Update" && id) {
            CommonFn.Ajax(me.Url + "/" + id, resources.Method.Put, data, function(response) {
                if(response) {
                    ...
                    alert("Chỉnh sửa thất bại, hãy chỉnh sửa lại thông tin!");
                }
            })
        }
    }
    ...
    resetFormUpdate(param) {
        ...
        if(param) {
            me.form.find("[setField]").each(function() {
                ...
                // Chuyển đổi giá trị setFeild để map với dữ liệu trong data
    ...
    resetFormReplication(param) {
        ...
        if(param) {
            me.form.find("[setField]").each(function() {
                ...
                // Chuyển đổi giá trị setFeild để map với dữ liệu trong data
                setFeild = setFeild.charAt(0).toLowerCase() + setFeild.slice(1);
                param.data[setFeild] = formatDataInput(setFeild, param.data[setFeild], param.employeeCode);
    ...
    // Hàm format data hiển thị lên Input
    formatDataInput(setFeild, data, employeeCode) {
        if(setFeild === "phoneNumber") {
            data = "0" + data.slice(5).trim();
        }
        else if(setFeild === "dateOfBirth") {
            data = CommonFn.formatDateInput(data);
        }
        else if(setFeild === "employeeCode") {
            data = employeeCode;
        }
        return data;
    }
